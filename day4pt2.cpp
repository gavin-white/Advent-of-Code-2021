...
void solution(std::ifstream & input) {
  ...
  std::vector<int> winner_idxs;

  for (int num : bingo_nums) {
    for (int j = 0; j < boards.size(); j++) {
      if (std::find(winner_idxs.begin(), winner_idxs.end(), j) != winner_idxs.end())
        continue;
      ...
      // this solution isn't great--boards get checked many extra times
      if (score != -1) {
        last_winner_score = score;
        last_num_winner = num;
        winner_idxs.push_back(j);
      }
    }
  }
  
  std::cout << last_num_winner * last_winner_score << std::endl;
}
